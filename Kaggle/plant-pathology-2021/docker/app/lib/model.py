...
from os.path import exists, isfile, abspath, getctime
...
from .image import show_img, spectrum, compare, opencv2tensorflow, \
...
class Model:
    ...
    def __init__(self, **kwargs):
        ...
        if self.CSV_PATH and not self.DEVICES:
            self.DEVICES = self.init_devices(memory_limit=self.MEMORY_LIMIT,
                                             memory_growth=self.MEMORY_GROWTH,
                                             only_cpu=self.ONLY_CPU,
                                             verbose=self.VERBOSE)

    def fit_check_save(self):
        """Фитирует модель, проверяет предсказательную силу, сохраняет модель в файл"""
    ...
    def predict(self,
        ...
        :param result_prc: Вывод результата совпадений с классами в процентах,
                           а не в логическом True/False
        ...
        if classes:
            ...
            if classes_thresholds:
                if classes_thresholds_prc:
                    for cls, val in classes_thresholds_prc.items():
                        prediction[cls] = 100 * prediction[cls] / val
                        prediction[cls] = prediction[cls].round()
                        prediction[prediction[cls] > 100] = 100
                else:
    ...
    def get_ds_df(self,
        ...
        if '.csv' not in str(csv_path):  # если идёт предсказание по одному фото, то передаётся путь к изображению вместо CSV файла
            source = pd.DataFrame({'labels': ['none'], 'image': [csv_path]})
            img_folder = ''
        else:
    ...
    def get_rate_threshold(self,
        ...
        # UPD: таблица результатов (в переменной cls_pred) вполне подходит для оценки
        # модели, график не так уж и нужен.
        # Чтобы избежать ошибки, надо проверять кол-во уникальных значений:
        cls_pred = None
    ...
    def model_save(self,
        ...
        # Поэтому его лучше делать после сохранения файла модели,
    ...
    def get_list(self) -> list:
        ...
        """
        paths = [f for f in list_files_in_dir(f'models/*/info')]
        ...
        for path in paths:
            ...
            info['path'] = abspath(path).rstrip('/info')  # абсолютный путь к файлу модели
    ...
    def predict_file(self,
                     path_file:str='',
                     path_model:str=''):
        """Предсказываем значение для одного изображенияя, используя последнюю созданную модель.
        :param str path_file: путь к файлу изображения, по которому надо сделать предсказание
        :param str path_model: путь к папке с моделью
        :return int: предсказанное значение
        """
        if not path_model:
            path_model = self.get_last()['path']

        try:
            # Загружаем модель, чтобы сделать предсказание для одного фото
            model_tensor, info, predict_sample = self.model_load(
            ...
            self.STRATEGY = info['preprocess_strategy']
            predict_ds, self.DF = self.get_ds_df(csv_path=self.CSV_PATH,
                                         img_folder=self.IMG_FOLDER,
                                         # img_folder_augmented=self.IMG_FOLDER_AUGMENTED,
                                         limit_classes=self.LIMIT_CLASSES,
            ...
            # Сделаем предсказание
            prediction = self.predict(model_tensor,
                                       predict_ds,
                                       df=self.DF,
                                       classes=classes,
                                       classes_thresholds=info['threshold']['val'],
                                       classes_thresholds_prc=info['threshold']['max'],
                                       verbose=self.VERBOSE)

            res = prediction[info['classes']].T.to_dict()[0]
            return {'values': res}
        except:
            pass
