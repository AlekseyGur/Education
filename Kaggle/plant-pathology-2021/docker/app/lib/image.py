...
from imghdr import what as imwhat


def validate_image(stream):
    """Подтверждает, что файл действительно является изображением.
    В случае успешного подтверждения возвращает расширение.
    Иначе None"""
    header = stream.read(512)
    stream.seek(0)
    format = imwhat(None, header)
    if not format:
        return None
