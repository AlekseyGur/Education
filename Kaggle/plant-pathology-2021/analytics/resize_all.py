...
def calculation_single_process(*args):
    ...
    for idx, file_path in enumerate(paths):
        save_to = file_path.replace(f'source/train_images/',
                                    # f'source/train_images_{model.STRATEGY}/')
                                    f'source/train_images_{model.IMG_WIDTH}_{model.IMG_HEIGHT}_{model.STRATEGY}/')

        if not os.path.isfile(save_to):
            # print(f'Обрабатываем: {save_to}')
            # img = model.get_image(file_path=file_path,
...
for width, height in [[500, 350], [800, 600], [600, 400]]:
    ...
    for strategy in strategies:
        ...
        directory = f'source/train_images_{img_size}_{strategy}/'
        ...
        model = Model(
            ...
            IMG_FOLDER=f'source/train_images/',  # Путь к папке с изображениями
