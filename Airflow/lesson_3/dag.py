...
TELEGRAM_CHAT_ID = 123456789  # get from botFather
TELEGRAM_CONN_ID_ADMIN = ''  # setup in airflow admin panel
...
with DAG(
    ...
    with TaskGroup("telegram_notice") as telegram_notice:
        show_titanic_mean_fares = BashOperator(
            ...
            bash_command = '''beeline -u jdbc:hive2:localhost:10000 -e "SELECT * FROM titanic_mean_fares LIMIT 5" | tr "\n" ";" '''
        )
        
        show_titanic_pivot = BashOperator(
            task_id='show_titanic_pivot',
            bash_command = '''beeline -u jdbc:hive2:localhost:10000 -e "SELECT * FROM titanic_pivot LIMIT 5" | tr "\n" ";" '''
        )
        ...
        def format_message(**kwargs):
            ...
            # return text # можно делать просто возврат
        ...
        send_result = TelegramOperator(
            ...
            telegram_conn_id=TELEGRAM_CONN_ID_ADMIN,
