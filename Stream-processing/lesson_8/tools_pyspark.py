...
def console_stream(stream_or_df, processingTime='1 seconds', mode='append'):
    """Пишет поток writeStream или DataFrame в файловую консоль"""
    if str(type(stream_or_df)) == "<class 'pyspark.sql.dataframe.DataFrame'>":
        return stream_or_df.write.format('json') \
                        .mode('append') \
                        .save('console')
    else:
        return stream_or_df.writeStream \
                .format('json') \
                .option('path', 'console') \
